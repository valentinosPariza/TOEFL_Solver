...
public class StringManipulation {
	 ...
	 * 
	 * 
	 * @param words an array type of String[] which is used to get the data in it and place them into a bigger array
	 * @return a String[] array with all the data from the array words,but  bigger capacity 
	 * @author Valentinos Pariza
	 */
	private static String[] expand(String[] words)
	{
	...
	/**
	 * 
	 * @param array a String[] array to be trimmed
	 * @param length the current number of the Strings in the array
	 * @return a String[] array which is an array with length as indicated and all the data from the array(from 
	 *  	   position 0-->length-1) which are given into the array.
	 * @author Valentinos Pariza
	 */
	...
	/**
	 * 
	 * @param text a String which is given to split it into pieces
	 * @return a String[] array which is an Array which inside has into consecutive positions all the substrings 
	 * 		   (broken pieces by characters ' ','\n','\t' from the String which is given as parameter). 
	 * @author Valentinos Pariza
	 */
	public static String[] split(String text)
	{
		...
		if(c==' '||c=='\n'||c=='\t'||c=='\r'||c=='\f')	    // if the first character is a "white space" then it is 
		...
		for(int i=1;i<length;i++)			//Beginning from i=1 because the first character was already checked.
		{
			...
			if(c==' '||c=='\n'||c=='\t'||c=='\r'||c=='\f')
			{
				...
				if(previousC!=' '&&previousC!='\n'&&previousC!='\t'&&previousC!='\r'&&previousC!='\f')
				{
				...
				index=i+1;	 // if a "white space" was found then after checking, the index must be placed after 
							 // the "white space" in order to represent the position of a non "white space" character
			}
			
			
		}
		
		// After finishing the for-Loop,the variable c is the last character of the text.If that character is not a 
		// "white space" then the last String of the text was not placed inside the array.If so it places it.
